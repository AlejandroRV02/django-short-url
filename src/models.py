...
class EnlaceQuerySet(models.QuerySet):
    ...
    def fechas(self,pk):
        return self.values('fecha').annotate(julio=models.Sum('contador', filter=models.Q(fecha__gte=datetime.date(2019,7,1), fecha__lte=datetime.date(2019,7,31)))).filter(pk=pk)
...
class Enlace(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('src:enlace', kwargs={'pk':self.pk})